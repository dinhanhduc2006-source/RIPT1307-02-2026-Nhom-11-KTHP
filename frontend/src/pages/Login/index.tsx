...
const LoginContent: React.FC = () => {
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    try {
      ...
      message.loading('Đang xác thực với hệ thống...', 0);
      
      // Send the credential (ID Token) to your backend
      const response: any = await authApi.googleLogin(credentialResponse.credential);
      ...
      message.destroy();
      message.error(error?.message || 'Xác thực với hệ thống thất bại');
    }
  };
